...
public class MazeSearch {
    ...
    public final int DELAY = 3; //Changed delay time
    ...
    private boolean explore(Cell cell) {
        ...
        //for each iterates through the map of neighbors
        for (Cell n : neighbours.get(cell)) {
            //if Not visited runs the yellow path.
            if (!n.isVisited()) {
                drawPassage(cell, n, Color.yellow, true);
                //Nested for loop checks if the sell has been explored if so. draws red if backtracking
                //And draws a final green path when successfully finished.
                if (explore(n)) {
                    drawPassage(cell, n, Color.green, true);
                    return true;
                } else {
                    drawPassage(cell, n, Color.RED, true);
                }
            }
        }
        return false;
