...
public class Sokoban {
    ...
    private Stack<ActionRecord> undoStack; //Stack to undo the moves made
    ...
    public void doAction(String action) {
        ...
        if (squares[nextP.row][nextP.col].hasBox() && squares[nextNextP.row][nextNextP.col].isFree()) {
            ...
            undoStack.push(new ActionRecord("Push",workerDirection));
            ...
            undoStack.push(new ActionRecord("Move",workerDirection));
        }
    }
    ...
    /**
     * Basically this uses a Stack to reverse the moves made previously in the game. because of previous action records being saved and added to the stack
     * it adds it to the stack, then depending on the action saved the method decides between options. there is one issue with this method
     * it does not change the direction of the worker when reversed only his position.
     */
    public void undo() {
        if (!undoStack.isEmpty()) {  //Checks if the Stack is empty
            ActionRecord action = undoStack.pop();
            if (action.isPush() == true) {
                pull(oppositeDirection(action.direction()));
            } else if (action.isMove() == true) {
                move(oppositeDirection(action.direction()));
            } else {
                UI.println("Undo Unavailable");
            }
